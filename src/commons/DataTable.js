import React, { useState, useContext, useFocusRef, createContext, useMemo } from 'react';
...
const DataTable = ({ cols, rows, headerRowHeight, direction }) => {
    ...
    const renderFilters = (filterType, row) => {
        ...
        return [...filts].map((filter, index) => {
            if (filts.length === index) {
                if (filterType === 'text') {
                    return `${filts[row.name] ? row[row.name].includes(filts[row.name]) : true} &&`;
                } else if (filterType === 'option') {
                    return `${filts[row.name] !== '' ? row[row.name] === filts[row.name] : true} &&`;
                } else {
                }
            } else {
                if (filterType === 'text') {
    ...
    return (
        <FilterContext.Provider value={filters}>
            <DataGrid columns={columns} rows={filteredRows} headerRowHeight={headerRowHeight || 70} direction={direction || 'ltr'} />
